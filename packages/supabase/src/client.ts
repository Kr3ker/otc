...
export function createAnonClient(config?: SupabaseConfig): TypedSupabaseClient {
  const url = config?.url ?? process.env.NEXT_PUBLIC_SUPABASE_URL ?? LOCAL_SUPABASE_URL
  const key = config?.anonKey ?? process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY ?? LOCAL_ANON_KEY
 ...
 */
export function createServiceClient(config?: SupabaseConfig): TypedSupabaseClient {
  const url = config?.url ?? process.env.SUPABASE_URL ?? LOCAL_SUPABASE_URL
  const key = config?.serviceRoleKey ?? process.env.SUPABASE_SERVICE_ROLE_KEY ?? LOCAL_SERVICE_ROLE_KEY
